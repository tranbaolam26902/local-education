...
public static class TourEndpoints
{
	...
	{
		RouteGroupBuilder builder = app.MapGroup("/api/tours");
		...
		[FromServices] IMapper mapper)
	{
		try
		{
			TourQuery condition = mapper.Map<TourQuery>(model);

			condition.IsPublished = true;

			Core.Contracts.IPagedList<TourItem> pagedTours = await repo.GetPagedToursAsync(condition, model);

			PaginationResult<TourItem> paginationResult = new(pagedTours);
		...
		[FromServices] IMapper mapper)
	{
		try
		{
			...
			Core.Contracts.IPagedList<TourItem> pagedTours = await repo.GetPagedToursAsync(condition, model, user.Id);
		...
		[FromServices] IMapper mapper)
	{
		try
		{
			Tour tour = await repo.GetTourBySlugAsync(slug);
			...
			TourDto tourDto = mapper.Map<TourDto>(tour);
		...
		[FromServices] IMapper mapper)
	{
		try
		{
			...
			Tour tour = mapper.Map<Tour>(model);
			tour.UserId = user.Id;

			await repo.AddOrUpdateTourAsync(tour);

			TourItem result = mapper.Map<TourItem>(tour);
		...
		[FromServices] IMapper mapper)
	{
		try
		{
			Tour check = await repository.GetTourByIdAsync(tourId);
			if (check == null)
			{
				return Results.Ok(ApiResponse.Fail(HttpStatusCode.NotFound,
			...
			Atlas atlas = mapper.Map<Atlas>(model);

			atlas.Id = tourId;
			Atlas result = await repository.AddOrUpdateAtlasAsync(atlas);
		...
		[FromServices] IMapper mapper)
	{
		try
		{
			Tour tourCheck = await repository.GetTourByIdAsync(tourId);
			if (tourCheck == null)
			{
				return Results.Ok(ApiResponse.Fail(HttpStatusCode.NotFound,
					$"Tour không được tìm thấy với mã: '{tourId}'"));
			...
			IList<Scene> newScenes = mapper.Map<IList<Scene>>(models);

			List<Guid> listLessonsId = newScenes
				.SelectMany(scene => scene.InfoHotspots, (_, hotspot) => hotspot.LessonId)
				.Distinct()
				.ToList();


			foreach (Guid lessonId in listLessonsId)
			{
				Lesson lesson = await lessonRepository.GetLessonByIdAsync(lessonId);
				if (lesson == null)
				{
					return Results.Ok(ApiResponse.Fail(HttpStatusCode.NotFound,
			...
			Tour tour = await repository.AddOrUpdatesSceneAsync(tourId, newScenes);
		...
		[FromServices] IMapper mapper)
	{
		try
		{
			Tour tour = await repo.GetTourByIdAsync(tourId);
			...
			TourItem tourDto = mapper.Map<TourItem>(tour);
		...
		[FromServices] ITourRepository repository)
	{
		try
		{
			Tour tour = await repository.GetTourByIdAsync(tourId);
			...
			return await repository.TogglePublicStatusAsync(tourId)
				? tour.IsPublished
					? Results.Ok(ApiResponse.Success("Tour đã được chuyển sang riêng tư"))
					: Results.Ok(ApiResponse.Success("Tour đã được chuyển sang công khai"))
		...
		[FromServices] ITourRepository repository)
	{
		try
		{
			Tour tour = await repository.GetTourByIdAsync(tourId, true);
			if (tour == null)
			{
				return Results.Ok(ApiResponse.Fail(
					HttpStatusCode.NotFound,
			...
			return await repository.ToggleDeleteStatusAsync(tourId)
				? tour.IsDeleted
					? Results.Ok(ApiResponse.Success("Tour đã được khôi phục từ thùng rác"))
					: Results.Ok(ApiResponse.Success("Tour đã được chuyển vào thùng rác"))
				: Results.Ok(ApiResponse.Success("Chuyển đổi trạng thái thất bại"));
		}
		catch (Exception e)
		{
			return Results.Ok(ApiResponse.Fail(HttpStatusCode.BadRequest, e.Message));
		...
		[FromServices] ITourRepository repository)
	{
		try
		{
			Tour tour = await repository.GetTourIsDeletedAsync(tourId);
			if (tour == null)
			{
				return Results.Ok(ApiResponse.Fail(
					HttpStatusCode.NotFound,
